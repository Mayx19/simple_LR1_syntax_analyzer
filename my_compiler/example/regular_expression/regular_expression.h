...
#define _add_next_state                                                                  \
    ...
    {                                                                                    \
        printf("goto[I_%d][%s]:", curr_state->parent->i_state, syntax_get_v(syn, find)); \
        if (!cnt)                                                                        \
        {                                                                                \
            ...
            {                                                                            \
                printf("[NEXT ACT]");                                                    \
                _printf_state_node(curr_state, str_temp);                                \
                printf("\n");                                                            \
            }                                                                            \
        }                                                                                \
        else                                                                             \
...
{
    printf("loop_check\n\tcurr:<I-%d depth:%d pos%d>\n", curr->i_state, curr->depth, curr->str_pos);
    /* printf("<I-%d depth:%d pos%d> next %d\n", (flag + curr->st r_pos + 1)->i_state, (flag + curr->st r_pos + 1)->depth, (flag + curr->str_ pos + 1)->str_ pos, (flag + curr->str_ pos + 1)->next_act);*/
    ...
    {
        // printf("\t\t<I-%d depth:%d pos%d>\n", temp->i_state, temp->depth, temp->str_pos);
        temp = (_match_state *)temp->next_act.next;
    }
...
#define find_vt                                                                         \
    {                                                                                   \
        ...
        cnt++;                                                                          \
        /*following code is for possible empty vn input*/                               \
...
#define DEBUG_PRINT_STATE_STACK                                                          \
    ...
    {                                                                                    \
        ...
        printf("\n\tPOS[ %d ] = %c\n", str_pos, str[str_pos] == 0 ? '#' : str[str_pos]); \
    }
...
#define __pop_match_state_stack__                                                                \
    ...
    {                                                                                            \
        ...
        IFDEBUG printf("\t[POP] stack %d <I-%d depth:%d pos%d>\n",                               \
                       curr_state, curr_state->i_state, curr_state->depth, curr_state->str_pos); \
        break;                                                                                   \
    }                                                                                            \
    else                                                                                         \
    {                                                                                            \
    ...
    int *pos)
{
    ...
    int str_pos = 0, str_pos_pre = 0, reslen = 0, flag_max = -1, result = -1, cnt = 0;
    ...
    while (1)
    {
        ...
        case _ACTION_GOTO_OP_MOVE_FORWORD:
            ...
            str_pos = curr_state->str_pos + 1;
            IFDEBUG printf("\n\t[MOVE] I_%d pos:%d", curr_state->i_state, str_pos);
            if (str_pos <= flag_max)
            {
